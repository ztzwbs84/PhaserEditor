...
export class AudioService {
  ...
  playPickup(combo: number): void {
    this.playTone(260 + Math.min(combo, 8) * 35, 0.07, 'sine')
  }

  playImpact(): void {
    this.playTone(92, 0.14, 'sawtooth')
  }
...
export class GameServices {
  ...
  publishSnapshot(snapshot: RunSnapshot): void {
    this.profile.observeRun({ phase: snapshot.phase, terminalKind: snapshot.terminalKind, progress: snapshot.score })
    this.publishProfile()
