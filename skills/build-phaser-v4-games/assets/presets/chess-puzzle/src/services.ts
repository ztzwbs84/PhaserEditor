...
export class AudioService {
  ...
  playSelect(): void { this.playTone(360, 0.05, 'sine') }
  playCorrect(puzzle: number): void { this.playTone(520 + puzzle * 80, 0.16, 'triangle') }
  playWrong(): void { this.playTone(140, 0.18, 'square') }
  playEnd(won: boolean): void { this.playTone(won ? 840 : 92, 0.34, won ? 'triangle' : 'sawtooth') }
...
export class GameServices {
  ...
  publishSnapshot(snapshot: RunSnapshot): void {
    this.profile.observeRun({ phase: snapshot.phase, terminalKind: snapshot.terminalKind, progress: snapshot.puzzles })
    this.publishProfile()
