...
services.bus.on(GAME_EVENTS.snapshot, (snapshot: RunSnapshot, selectedTurret: number) => {
  controls.setSnapshot(snapshot, selectedTurret)
  if (snapshot.phase === 'game-over') controls.announce(snapshot.terminalKind === 'success'
    ? `Sector defended. ${snapshot.intercepts} invaders intercepted. Score ${snapshot.score}`
    : `Citadel core breached. ${snapshot.intercepts} invaders intercepted. Score ${snapshot.score}`)
  else controls.announce('Activate a turret node before the invader reaches the core')
})
