...
const ASSETS = [
  ['citadel', 'assets/citadel.svg'],
  ['turret', 'assets/turret.svg'],
  ['invader', 'assets/invader.svg'],
...
export class BootScene extends Phaser.Scene {
  ...
  preload(): void {
    ...
    this.add.text(480, 236, __GAME_TITLE_JSON__, {
      color: '#ffcc66', fontFamily: 'Arial, sans-serif', fontSize: '34px', fontStyle: 'bold'
    }).setOrigin(0.5)
    this.progressText = this.add.text(480, 286, '0%', {
      color: '#effbe7', fontFamily: 'Arial, sans-serif', fontSize: '18px'
    }).setOrigin(0.5)
  ...
  create(): void {
    ...
    if (this.failedFiles > 0) {
      ...
      const retry = this.add.text(480, 330, 'Retry', {
        backgroundColor: '#d95d62', color: '#111813', fontFamily: 'Arial, sans-serif', fontSize: '18px', padding: { x: 18, y: 10 }
