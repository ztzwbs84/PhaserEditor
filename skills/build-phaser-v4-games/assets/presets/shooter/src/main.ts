...
services.bus.on(GAME_EVENTS.snapshot, (snapshot: RunSnapshot) => {
  ...
  if (snapshot.phase === 'game-over') controls.announce(snapshot.terminalKind === 'success'
    ? `Sector clear. ${snapshot.kills} raiders destroyed. Score ${snapshot.score}`
    : `Defense line lost. ${snapshot.kills} raiders destroyed. Score ${snapshot.score}`)
  else controls.announce('Fire at the raider before it crosses the defense line')
})
