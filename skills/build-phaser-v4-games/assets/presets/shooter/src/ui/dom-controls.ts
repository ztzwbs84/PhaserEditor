...
import { RUN_COMPLETION_TARGET } from '../domain/run-model'
...
export class DomControls {
  ...
  private readonly objective = this.requiredOutput('sortie-objective')
  private readonly chain = this.requiredOutput('sortie-chain')
  private readonly score = this.requiredOutput('sortie-score')
  private readonly remaining = this.requiredOutput('sortie-remaining')
  ...
  setSnapshot(snapshot: RunSnapshot): void {
    this.progress.value = String(snapshot.kills)
    ...
    this.objective.value = snapshot.phase === 'game-over'
      ? snapshot.terminalKind === 'success' ? 'Sector clear' : 'Defense line lost'
      : snapshot.phase === 'paused' ? 'Sortie paused' : 'Destroy nearest raider'
    this.chain.value = `x${snapshot.chain}`
    this.score.value = String(snapshot.score)
    this.remaining.value = String(Math.max(0, RUN_COMPLETION_TARGET - snapshot.kills))
  }
