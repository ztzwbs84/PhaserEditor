...
export class AudioService {
  ...
  playShot(): void { this.playTone(620, 0.045, 'square') }
  playHit(): void { this.playTone(185, 0.08, 'sawtooth') }
  playDestroy(chain: number): void { this.playTone(330 + Math.min(chain, 8) * 45, 0.12, 'triangle') }
  playDamage(): void { this.playTone(82, 0.18, 'sawtooth') }
  playEnd(cleared: boolean): void { this.playTone(cleared ? 470 : 96, 0.3, cleared ? 'triangle' : 'sawtooth') }
...
export class GameServices {
  ...
  publishSnapshot(snapshot: RunSnapshot): void {
    this.profile.observeRun({ phase: snapshot.phase, terminalKind: snapshot.terminalKind, progress: snapshot.kills })
    this.publishProfile()
