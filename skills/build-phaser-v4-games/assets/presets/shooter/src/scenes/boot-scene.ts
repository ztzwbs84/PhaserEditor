...
const ASSETS = [
  ['starfield', 'assets/starfield.svg'],
  ['fighter', 'assets/fighter.svg'],
  ['raider', 'assets/raider.svg'],
...
export class BootScene extends Phaser.Scene {
  ...
  preload(): void {
    ...
    this.add.text(480, 236, __GAME_TITLE_JSON__, {
      color: '#ffcb77', fontFamily: 'Arial, sans-serif', fontSize: '34px', fontStyle: 'bold'
    }).setOrigin(0.5)
    this.progressText = this.add.text(480, 286, '0%', {
      color: '#e9fff9', fontFamily: 'Arial, sans-serif', fontSize: '18px'
    }).setOrigin(0.5)
  ...
  create(): void {
    ...
    if (this.failedFiles > 0) {
      ...
      const retry = this.add.text(480, 330, 'Retry', {
        backgroundColor: '#ff5d73', color: '#07131b', fontFamily: 'Arial, sans-serif', fontSize: '18px', padding: { x: 18, y: 10 }
