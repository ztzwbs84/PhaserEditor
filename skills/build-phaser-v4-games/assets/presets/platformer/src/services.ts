...
export class AudioService {
  ...
  playJump(): void { this.playTone(290, 0.07, 'square') }
  playRelic(chain: number): void { this.playTone(460 + Math.min(chain, 8) * 34, 0.12, 'triangle') }
  playImpact(): void { this.playTone(96, 0.16, 'sawtooth') }
  playEnd(completed: boolean): void { this.playTone(completed ? 390 : 112, 0.28, completed ? 'triangle' : 'sawtooth') }
...
export class GameServices {
  ...
  publishSnapshot(snapshot: RunSnapshot, heightMeters = 0): void {
    this.profile.observeRun({ phase: snapshot.phase, terminalKind: snapshot.terminalKind, progress: snapshot.relics })
    this.publishProfile()
    this.bus.emit(GAME_EVENTS.snapshot, snapshot, heightMeters)
  }
  async destroy(): Promise<void> {
