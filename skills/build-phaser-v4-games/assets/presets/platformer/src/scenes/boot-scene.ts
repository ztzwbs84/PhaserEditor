...
export class BootScene extends Phaser.Scene {
  ...
  preload(): void {
    this.failedFiles = 0
    this.cameras.main.setBackgroundColor('#17151d')
    this.add.text(480, 236, __GAME_TITLE_JSON__, {
      color: '#ffd166', fontFamily: 'Georgia, serif', fontSize: '34px', fontStyle: 'bold'
    }).setOrigin(0.5)
    this.progressText = this.add.text(480, 286, '0%', {
      color: '#f7f4ed', fontFamily: 'Arial, sans-serif', fontSize: '18px'
    }).setOrigin(0.5)
  ...
  create(): void {
    ...
    if (this.failedFiles > 0) {
      ...
      this.progressText?.setText('Assets unavailable')
      const retry = this.add.text(480, 330, 'Retry', {
        backgroundColor: '#ef476f', color: '#17151d', fontFamily: 'Arial, sans-serif', fontSize: '18px', padding: { x: 18, y: 10 }
