...
services.bus.on(GAME_EVENTS.snapshot, (snapshot: RunSnapshot, heightMeters: number) => {
  controls.setSnapshot(snapshot, heightMeters)
  if (snapshot.phase === 'game-over') controls.announce(snapshot.terminalKind === 'success'
    ? `Ascent complete. ${snapshot.relics} relic${snapshot.relics === 1 ? '' : 's'}. Score ${snapshot.score}`
    : `Ascent lost. ${snapshot.relics} relic${snapshot.relics === 1 ? '' : 's'}. Score ${snapshot.score}`)
  else controls.announce('Jump to the next relic and avoid the spikes')
})
...
const game = new Phaser.Game({
  ...
  physics: { default: 'arcade', arcade: { gravity: { x: 0, y: 980 }, debug: false } },
