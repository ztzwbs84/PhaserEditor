...
export class AudioService {
  ...
  playBoost(): void { this.playTone(330, 0.06, 'sawtooth') }
  playGate(streak: number): void { this.playTone(440 + Math.min(streak, 8) * 52, 0.14, 'triangle') }
  playCrash(): void { this.playTone(92, 0.2, 'square') }
  playEnd(won: boolean): void { this.playTone(won ? 720 : 74, 0.32, won ? 'triangle' : 'sawtooth') }
...
export class GameServices {
  ...
  publishSnapshot(snapshot: RunSnapshot, speed: number): void {
    this.profile.observeRun({ phase: snapshot.phase, terminalKind: snapshot.terminalKind, progress: snapshot.checkpoints })
    this.publishProfile()
    this.bus.emit(GAME_EVENTS.snapshot, snapshot, speed)
  }
