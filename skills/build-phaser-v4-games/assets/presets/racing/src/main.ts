...
services.bus.on(GAME_EVENTS.paused, (paused: boolean, automatic: boolean) => {
  ...
  controls.announce(paused ? (automatic ? 'Race paused after losing focus' : 'Race paused') : 'Race resumed')
})
services.bus.on(GAME_EVENTS.snapshot, (snapshot: RunSnapshot, speed: number) => {
  controls.setSnapshot(snapshot, speed)
  if (snapshot.phase === 'game-over') controls.announce(snapshot.terminalKind === 'success'
    ? `Lap complete. ${snapshot.checkpoints} gates cleared. Score ${snapshot.score}`
    : `Hover chassis wrecked. ${snapshot.checkpoints} gates cleared. Score ${snapshot.score}`)
  else controls.announce(`Accelerate toward checkpoint ${Math.min(snapshot.checkpoints + 1, 4)}`)
})
