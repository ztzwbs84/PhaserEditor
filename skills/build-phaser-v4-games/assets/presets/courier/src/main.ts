...
services.bus.on(GAME_EVENTS.snapshot, (snapshot: RunSnapshot) => {
  ...
  if (snapshot.phase === 'game-over') controls.announce(snapshot.terminalKind === 'success'
    ? `Routes complete. ${snapshot.deliveries} deliveries. Score ${snapshot.score}`
    : `Shift lost. ${snapshot.deliveries} deliveries. Score ${snapshot.score}`)
  else if (snapshot.cargo) controls.announce(`Deliver the ember to the ${snapshot.cargo.destination} beacon`)
  else controls.announce('Return to the central hearth for another ember')
})
...
const game = new Phaser.Game({
  ...
  scale: { mode: Phaser.Scale.FIT, autoCenter: Phaser.Scale.CENTER_HORIZONTALLY, width: 960, height: 600 },
